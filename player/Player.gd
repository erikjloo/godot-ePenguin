...
func _apply_movement(delta : float) -> void:
  ...
  acceleration = ACCELERATION - FRICTION*int(Input.is_action_pressed("crouch")) 
...
func _update_stamina() -> void:
  var increment : int = -1 if !is_grounded or is_running else 1
...
func _jump_or_fly(speed : float = max_jump_speed) -> void:
  if Input.is_action_pressed("ui_down"):
    set_collision_mask_bit(DROP_THRU_BIT, false)  # Fall through platform
  elif is_grounded or !coyote_timer.is_stopped():
  ...
  elif stamina.value and flutter_timer.is_stopped():
...
func can_stand() -> bool:
  ...
  for i in range(results.size() - 1, -1, -1):
    var collider : CollisionObject2D = results[i].collider
